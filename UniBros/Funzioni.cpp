...
void WaitScreen()
{
	// - Funzione che visualizza una schermata di attesa
...
void Movement(bool* gameOver, Player& p, Screen& schermo)
{
	 ...
	 * Il giocatore 'p' viene passato per riferimento e viene mosso all'interno
	 * dello 'schermo' grazie a '_kbhit()', che si occupa proprio di gestire la
	 * console per gli input utente.
	 */
...
{
	// - Funzione che gestisce le azioni e gli attributi del giocatore
	...
	if (money.getType(p.getX(),p.getY()) == 1)
		p.setVite(p.getVite() + 1);
	if (money.getType(p.getX(), p.getY()) == 2)
		p.addAmmo(1);
