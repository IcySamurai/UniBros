			...
			case 2:		// Nemici che possono sparare
			{
				...
				if (list->movementSleepCount_UpDown == 3 && (list->direction == 0 || list->direction == 1))
				{
					...
					if (list->direction == 0)	// Spara verso il basso
					{
						...
						{
							if (!(generablePositions.isThere(list->x, list->y + 2)) && !(list->directionChanged))
							{
								list->direction = 1;
								list->directionChanged = true;
							}
				...
				else if (list->movementSleepCount_RightLeft == 1 && (list->direction == 2 || list->direction == 3))
				{
					...
					if (list->direction == 2)	// Spara verso destra
					{
						...
						{
							if (list->x >= generablePositions.getWidth() && !(list->directionChanged))
							{
								list->direction = 3;
								list->directionChanged = true;
							}
					...
					else if (list->direction == 3)	// Spara verso sinistra
					{
						...
						{
							if (list->x <= 0 && !(list->directionChanged))
							{
								list->direction = 2;
								list->directionChanged = true;
							}
...
bool Enemy::enemyHandler(Player player, Bullet bullet)
{
	...
	while (copy != NULL)
	{
		...
		{
			if (player.getPrevy() >= copy->y && player.getX() == copy->x)	// Controllo se il giocatore tocca il nemico senza caderci da sopra
				touched = true;
